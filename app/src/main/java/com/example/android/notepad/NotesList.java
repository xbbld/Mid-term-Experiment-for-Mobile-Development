...
public class NotesList extends ListActivity {
    private static int int_for_collaction = 0;
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.menu_collaction:
                if (int_for_collaction == 0) {
                    performSearch_for_collaction();
                } else {
                    refresh_list();
                }return true;
...
//END_INCLUDE(copy)
    ...
    private void performSearch(String searchText) {
        ...
        if (cursor != null) {
            NotesCursorAdapter adapter
                    ...
                    cursor,
                    new String[] {NotePad.Notes.COLUMN_NAME_TITLE, NotePad.Notes.COLUMN_NAME_CREATE_DATE,NotePad.Notes.COLUMN_NAME_NOTE},
    ...
    private void performSearch_for_collaction() {


        String[] projection = {NotePad.Notes._ID, NotePad.Notes.COLUMN_NAME_TITLE, NotePad.Notes.COLUMN_NAME_NOTE, NotePad.Notes.COLUMN_NAME_CREATE_DATE,NotePad.Notes.COLUMN_NAME_COLLACTION};
    ...
    private void refresh_list() {
        ...
        // The names of the cursor columns to display in the view, initialized to the title column
        String[] dataColumns = {NotePad.Notes.COLUMN_NAME_TITLE, NotePad.Notes.COLUMN_NAME_CREATE_DATE,NotePad.Notes.COLUMN_NAME_NOTE};
    ...
    public void onClickCollaction(View view) {
        ...
        // 刷新列表视图
        getListView().invalidateViews(); // 或者重新查询数据库并设置新的适配器
    // 更新按钮文本
    ImageButton imageButton = (ImageButton) parentView.findViewById(R.id.button);
    int collaction = cursor.getInt(cursor.getColumnIndexOrThrow(NotePad.Notes.COLUMN_NAME_COLLACTION));
        imageButton.setImageResource(collaction == 1 ? R.drawable.baseline_star_rate_white_24dp : R.drawable.baseline_star_border_white_24dp);
    }
